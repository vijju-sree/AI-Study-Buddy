...
def record_audio(path):
    ...
    try:
        ...
        st.warning("⚠ Microphone not available. Please upload audio instead.")
        return False
...
def run():
    ...
    # ----------- RECORD (LOCAL ONLY) -----------
    st.header("🎤 Record Voice (Local Only)")
    audio_path = None
    if st.button("Start Recording"):
        audio_path = os.path.join(AUDIO_DIR, f"record_{datetime.now().strftime('%Y%m%d_%H%M%S')}.wav")
        ...
        if success:
            ...
            st.info("Use 'Upload Audio' below instead.")

    # ----------- UPLOAD AUDIO -----------
    st.header("📁 Upload Audio File")
    uploaded_file = st.file_uploader("Upload audio", type=["wav", "mp3"])
    if uploaded_file:
        upload_path = os.path.join(AUDIO_DIR, uploaded_file.name)
